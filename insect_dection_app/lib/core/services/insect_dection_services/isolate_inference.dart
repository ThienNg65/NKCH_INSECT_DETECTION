...
import 'package:insect_dection_app/core/model/inference_output.dart';
import 'package:insect_dection_app/core/utils/images_processing/images_processing.dart';
...
class IsolateInference {
  ...
  static void entryPoint(SendPort sendPort) async {
    ...
    await for (final InferenceModel isolateModel in port) {
      ...
      final imageMatrix = List.generate(
        ...
        (y) => List.generate(
          ...
          (x) {
            ...
            return [
              (pixel.r.toDouble() - 0) / 255.0,
              (pixel.g.toDouble() - 0) / 255.0,
              (pixel.b.toDouble() - 0) / 255.0
            ]; // div to 255 to perform scaling in range [0;1];
          },
        ),
      );
      ...
      var classification = <InferenceOutput>{};
      for (var i = 0; i < result.length; i++) {
        if (result[i] != 0) {
          // Set label: points
          InferenceOutput inferenceOutput = InferenceOutput(
            i.toString(),
            isolateModel.labels[i],
            result[i].toDouble() / maxScore.toDouble(),
          );
          classification.add(inferenceOutput);
        }
      }
